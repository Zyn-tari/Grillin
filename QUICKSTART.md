...
| **4** | Does "done" produce something that runs or deploys? | Mind the order: build → promote → restart → check. Gate the new thing, not the old one. | No ordering trap. **"Done" is still a command** — a `grep` for the thing that proves it, a count, a `test`. Not executable ≠ not checkable. Grade the CONTENT: `test -s f` is satisfied by writing any file at all, so it gates paperwork, not the fix. |
